...
class MoviesService:
    ...
    async def add(self, user_id: str, movie_id: str, score: int):
        ...
        if movie:
            ...
            return await self.stor.update({"_id": movie_id}, {"$addToSet": {"scores": {"user_id": user_id, "score": score}}})
        return await self.stor.insert({"_id": movie_id, "scores": [{"user_id": user_id, "score": score}], "rating": score})
    ...
    async def update(self, user_id: str, movie_id: str, score: int):
        ...
        if movie:
            ...
            await self.stor.update({"_id": movie_id}, {"$set": {"rating": rating}})
            return await self.stor.update({"_id": movie_id, "scores.user_id": user_id}, {"$set": {"scores.$.score": score}})
        return None
